  ...
  if (util? || app_server?) && !solo?
    ...
    node['applications'].each do |app_name,data|
      node[:nfs][:links].each do |link|
        bash "move-linked-dir" do
          user node[:owner_name]
          code "mv /data/#{app_name}/shared/#{link} /data/#{app_name}/shared/#{link}.bak"
          only_if { File.exists?("/data/#{app_name}/shared/#{link}") && !File.symlink?("/data/#{app_name}/shared/#{link}") }
        end
