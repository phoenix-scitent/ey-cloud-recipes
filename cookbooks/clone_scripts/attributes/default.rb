clone_scripts(
  ...
  'weekday_to_run' => "1",                                           # The day of week at which the full script occurs, "0" is Sunday (0-6)
  'database' => {
    :backup_bucket => "scitent-clone-51041190d661",                  # A random bucket name to store the latest db dump for the clone
    :backup_hour => "6",                                             # The hour at which the latest db dump should be uploaded to S3 (UTC)
    :restore_hour => "6"                                             # The hour at which the latest db dump should be downloaded from S3 and imported (UTC)
  },
  'deploy' => {
    :hour => "6",                                                    # The hour at which the automated deploy should happen (UTC)
    :branch => "master-data-test"                                    # The branch to deploy for the automated deploy
  },
  'rsync' => {
    :hour => "2",                                                    # The hour at which to begin the rsync (UTC)
